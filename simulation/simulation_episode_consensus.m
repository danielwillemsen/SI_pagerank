function action_steps = simulation_episode_consensus(sml, pattern, Q, varargin)
% simulation_episode_consensus perform one simulation episode of the task for a given policy Q and returns the number of actions taken.
%
% Mario Coppola, 2019
...
[state_local,state_local_full] = globalstate_to_observation_consensus(s0_l, sml.states, sml, pattern);
...
while action_steps < inf
    ...
    if selected_agent > n % If someone can move 
        ...
        if visualize
            plot_formation_consensus(sml.bw, s0_l, pattern);
            drawnow;
        end
